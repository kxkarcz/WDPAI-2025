...
class AppController
{
    ...
    protected function setFlash(string $key, string|array $message): void
    {
        $_SESSION['flash'][$key] = $message;
    }

    protected function getFlash(string $key, bool $remove = true): string|array|null
    {
    ...
    protected function requireCsrf(): void
    {
        if (!$this->validateCsrf()) {
            http_response_code(403);
            if ($this->wantsJson()) {
                $this->json(['error' => 'Nieprawidłowy token CSRF.'], 403);
            } else {
                $this->render('errors/403', ['title' => 'Nieprawidłowe żądanie']);
            }
            exit;
        }
    }
